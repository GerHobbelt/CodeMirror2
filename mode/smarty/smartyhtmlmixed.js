...
/*
z  function isStartingSmartyTag(stream, state) {
    if (/\{/.test(stream.current())) {
      state.token = smarty;
      state.localState = smartyMode.startState(htmlMode.indent(state.htmlState, ""));
      state.mode = "smarty";
...
//    } else
    ...
    return maybeBackupJS(stream, /<\/\s*script\s*>/,
  ...
  return {
    startState: function() {
      ...
      return {token: html, localState: null, mode: "html", htmlState: state, whatever: null };
    },
