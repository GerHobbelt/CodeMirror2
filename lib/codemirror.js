(function(mod) {
  ...
  function endOperation(cm) {
    ...
    if (op.viewChanged || op.forceUpdate || op.scrollTop != null ||
      ...
      var updated = updateDisplay(cm, op.viewChanged, {top: op.scrollTop, ensure: op.scrollToPos}, op.forceUpdate);
      ...
      var loadFile = function(file, i) {
        ...
        reader.onload = function() {
          ...
          if (++read == n) {
            ...
            setSelection(cm.doc, simpleSelection(pos, changeEnd(change)));
          }
        };
  ...
  function makeChangeFromHistory(doc, type, allowSelectionOnly) {
    ...
    var hist = doc.history, event;
    var source = type == "undo" ? hist.done : hist.undone, dest = type == "undo" ? hist.undone : hist.done;
    addSelectionToHistory(doc, doc.sel, dest, NaN);
    for (;;) {
      event = source.pop();
      if (!event) return;
      if (event.ranges) {
  ...
  function setSelection(doc, sel, scroll) {
    ...
    addSelectionToHistory(doc, sel, doc.history.done, doc.cm ? doc.cm.curOp.id : NaN);
  }
  ...
  LineWidget.prototype.clear = widgetOperation(function() {
    ...
    if (aboveVisible) addToScrollPos(this.cm, null, -this.height);
  ...
  Doc.prototype = createObj(BranchChunk.prototype, {
    ...
    replaceSelections: function(code, collapse, origin) {
      ...
      if (newSel) setSelection(this, newSel);
      else if (this.cm) ensureCursorVisible(this.cm);
    },
    undo: docOperation(function() {makeChangeFromHistory(this, "undo");}),
    redo: docOperation(function() {makeChangeFromHistory(this, "redo");}),
    undoSelection: docOperation(function() {makeChangeFromHistory(this, "undo", true);}),
    redoSelection: docOperation(function() {makeChangeFromHistory(this, "redo", true);}),
  ...
  function addSelectionToHistory(doc, cur, dest, opId) {
    var prev = lst(dest);
    if (prev && !prev.ranges) {
      if (doc.history.lastOp == opId)
        prev.selAfter = cur;
      else
        dest.push(cur);
    } else if (dest.length < 2 || prev.ranges.length != cur.ranges.length ||
               cur.somethingSelected() != prev.somethingSelected()) {
      dest.push(cur);
    } else {
      dest[dest.length - 1] = cur;
    }
  }
  ...
  function copyHistoryArray(events, newGroup, instantiateSel) {
    for (var i = 0, copy = []; i < events.length; ++i) {
      var event = events[i];
      if (event.ranges) {
        copy.push(copyHistorySel(event, instantiateSel));
        continue;
      }
  ...
  function rebaseHistArray(array, from, to, diff) {
    for (var i = 0; i < array.length; ++i) {
      ...
      if (sub.ranges) {
        ...
        rebaseHistSel(sub, from, to, diff);
        continue;
      }
